...
export function ExerciseCard({ exercise }: { exercise: Exercise }) {
  const primary = exercise.primaryMuscles.map((muscle) => MUSCLES_BY_ID[muscle]?.name).filter(Boolean);
  const secondary = exercise.secondaryMuscles.map((muscle) => MUSCLES_BY_ID[muscle]?.name).filter(Boolean);
  ...
  return (
    ...
    >
      <div className="relative aspect-[5/3] w-full overflow-hidden">
        <ExerciseThumb exercise={exercise} className="transition-transform duration-500 group-hover:scale-105" />
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <h3 className="font-display text-base font-semibold leading-tight text-foreground">{exercise.name}</h3>
